...
bondlength = 1;  % length of bonds between particles
a = get_adj(x,bondlength+tolD,dim);  % adjacency matrix
...
framework.lengths = a;    % desired distance matrix (same size as a)
                          % set to NAN if lengths are determined automatically from given lengths
